...
def inv(coordinate):
    if(coordinate[0] < 320):
        initial_joint_positions = [Vector2(320, 480), Vector2(380, 270), Vector2(440, 60)]
        tolerance = 0.02
        ...
        initial_joint_positions = [Vector2(320, 480), Vector2(180, 270), Vector2(40, 60)]
        tolerance = 0.02
        fab = pyfabrik.Fabrik2D(initial_joint_positions, tolerance)
        fab.move_to(Vector2(coordinate))
        x = fab.angles_deg
...
def get_angle(joints, coordinate):
    if(coordinate[0] < 320): #elbow up
        ...
        if joints[2].y > joints[1].y: #case 2
    ...
    if(coordinate[0] >= 320): #elbow down
        hypotenuse_1 = np.sqrt((joints[1].x - joints[0].x) ** 2 + (joints[0].y - joints[1].y) ** 2)
        adjacent_1 = joints[1].x - joints[0].x
        angle_0 = np.arccos(adjacent_1/hypotenuse_1) * (180/np.pi)
        angle_1 = 180 - angle_0

        if joints[2].y > joints[1].y: #case2
